...
router.post('/profile', upload.single('file'), async (req, res) => {
    try {
        ...
        // Example: we resize to max 1024 and save as webp for consistent storage (optional)
        ...
        const file = await File.create({
            ...
            versions: [
                {
                    key: `users/${userId}/profile/${thumbName}`,
                    versionNumber: 1,
...
router.get('/profile', async (req, res) => {
    try {
        ...
        // Find the File document for the user’s profile photo
        const file = await File.findOne({
            createdBy: requestedUser._id,
            type: 'profile_photo'
        }).sort({ createdAt: -1 }); // get the most recent if multiple exist
        ...
        const filepath = path.join(UPLOAD_ROOT, `users/${requestedUser.id}/profile`, path.basename(file.key));
